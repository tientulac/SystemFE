export const LIST_STATUS_CATEGORY_TYPE = [
    { id: 0, code: 'AwaitingApprove', name: 'Chờ duyệt' },
    { id: 1, code: 'Approve', name: 'Đã duyệt' },
    { id: 2, code: 'Show', name: 'Hiển thị' },
    { id: 3, code: 'Hide', name: 'Ẩn' },
];

export const LIST_STATUS_CATEGORY = [
    { id: 0, code: 'AwaitingApprove', name: 'Chờ duyệt' },
    { id: 1, code: 'Approve', name: 'Đã duyệt' },
    { id: 2, code: 'Show', name: 'Hiển thị' },
    { id: 3, code: 'Hide', name: 'Ẩn' },
];


export const LIST_STATUS_BLOG = [
    { id: 0, code: 'AwaitingApprove', name: 'Chờ duyệt' },
    { id: 1, code: 'Approve', name: 'Đã duyệt' },
    { id: 2, code: 'Show', name: 'Hiển thị' },
    { id: 3, code: 'Hide', name: 'Ẩn' },
];

export const LIST_TYPE_BLOG = [
    { id: 0, code: 'Text', name: 'Text' },
    { id: 1, code: 'File', name: 'File' },
];

export const LIST_STATUS_USER = [
    { id: 0, code: 'Disable', name: 'Ẩn' },
    { id: 1, code: 'Active', name: 'Kích hoạt' },
    { id: 2, code: 'Ban', name: 'Đã Khóa' },
    { id: 3, code: 'Approve', name: 'Duyệt' },
    { id: 4, code: 'Awaiting_Approve', name: 'Chờ duyệt' },
];
